...
func (s *Server) Routes() *gin.Engine {
	...
	auth.Use(s.withAuth())
	{
		...
		auth.GET("/test-sets/:id/background-documents", s.handleListBackgroundDocs)
		auth.POST("/test-sets/:id/background-documents", s.handleCreateBackgroundDoc)
		auth.GET("/test-sets/:id/background-documents/:docID", s.handleGetBackgroundDoc)
		auth.PATCH("/test-sets/:id/background-documents/:docID", s.handleUpdateBackgroundDoc)
		auth.DELETE("/test-sets/:id/background-documents/:docID", s.handleDeleteBackgroundDoc)

		auth.GET("/test-sets/:id/case-flows", s.handleListCaseFlows)
		...
		auth.POST("/case-flows/:caseFlowID/versions", s.handleSaveCaseFlowVersion)
		auth.GET("/case-flows/:caseFlowID/versions", s.handleListCaseFlowVersions)
		auth.GET("/case-flows/:caseFlowID/versions/:versionNo", s.handleGetCaseFlowVersion)
		auth.POST("/case-flows/:caseFlowID/versions/:versionNo/restore", s.handleRestoreCaseFlowVersion)
		auth.GET("/case-flows/:caseFlowID/sources", s.handleListCaseSources)
		auth.POST("/case-flows/:caseFlowID/sources", s.handleAddCaseSource)
		auth.DELETE("/case-flows/:caseFlowID/sources/:sourceID", s.handleRemoveCaseSource)
		auth.POST("/case-flows/:caseFlowID/nodes", s.handleAddCaseNode)
		auth.PATCH("/case-flows/:caseFlowID/nodes/:nodeID", s.handleUpdateCaseNode)
		auth.POST("/case-flows/:caseFlowID/nodes/:nodeID/move", s.handleMoveCaseNode)
		auth.DELETE("/case-flows/:caseFlowID/nodes/:nodeID", s.handleDeleteCaseNode)
		auth.PATCH("/case-flows/:caseFlowID/nodes/:nodeID/status", s.handleSetCaseStatus)
		auth.GET("/case-flows/:caseFlowID/nodes/:nodeID/flows", s.handleListNodeFlows)
		auth.GET("/case-flows/:caseFlowID/export", s.handleExportCaseFlow)

		auth.POST("/case-flows/:caseFlowID/agent/submit", s.handleCaseAgentSubmit)
		auth.GET("/case-flows/:caseFlowID/agent/session", s.handleCaseAgentSession)
		auth.POST("/case-flows/:caseFlowID/agent/resume", s.handleCaseAgentResume)
		auth.POST("/case-flows/:caseFlowID/agent/new", s.handleCaseAgentNew)
		auth.POST("/case-flows/:caseFlowID/agent/compress", s.handleCaseAgentCompress)
